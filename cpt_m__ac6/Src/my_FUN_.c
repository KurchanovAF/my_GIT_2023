...
#include "my_FUN.h"
	...
	//my_F1F2_();		//  969 тактов
	//my_F2F1_();		// 1057 тактов, измеренна€ сумма = 14808
...
void my_DataADC2_2_()
{
	// ѕри режекции используютс€ суммы с разносом в 5/2 периода
	// 165 = 3*55, 55 = 30 + 25, 30 = 12*5/2, 25 = 10*5/2
	// однако, замена 55 на 56 позвол€ет выровн€ть данные на границу слова
	//
	for(int i = 0; i < 56; i++)
	{
...
void my_F1_()
{
	...
	// јнализируем на участке длиной 120 отсчетов
	max_Data_F1_ = 0;
	min_Data_F1_ = 0;
	for(int i = 0; i < 120; i++)
	{
...
void my_F2_()
{
	...
	for(int i = 0; i < 30; i++)
	{
		my_DMA2_Data_F2_[i] =
			...
			my_DMA2_Data_F0_[i + 26];
			// 5/2 периода дл€ 12 к√ц = 25
			// ¬ј∆Ќќ - начало массива Data_F2_ нужно только
			// дл€ расчета массива Data_F1_F2_, а не дл€
			// синхронного детектировани€
		}
	// јнализируем на участке длиной 120 отсчетов
	max_Data_F2_ = 0;
	min_Data_F2_ = 0;
...
void my_F1F2_()
{
	// „астота 10 к√ц в Data_F1_[120] уже вырезана
	// ƒетектируем частоту 12 к√ц
	for(int i = 0; i < 5; i++)
	{
		p_my_F1F2_rez_[i] = 0;
	}
...
void my_F2F1_()
{
	// „астота 12 к√ц в Data_F2_[150] уже вырезана
	// ƒетектируем частоту 10 к√ц
	for(int i = 0; i < 6; i++)
	{
		p_my_F2F1_rez_[i] = 0;
	}
	for(int i = 0; i < 10; i++)
	{
		...
		// ѕервые 30 отсчетов в буфере _Data_F2_ мы здесь не используем, они нужны
		// дл€ построени€ буфера Data_F1_F2_ и анализа шумов и пульсаций лазера
	}
}
