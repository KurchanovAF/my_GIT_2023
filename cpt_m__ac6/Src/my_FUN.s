...
// работаем со свежими значениями, R2 - адрес в буфере my_F0 для OUT2
	...
	MOV R0, #60				// 480 = 120*4 = 8*60 = 4*2*60
	//R8, R9, R10 - свободны для использования
	LDR R8, my_0x0000FFFF	// константа
	MOV R9, #0				// сумматор для OUT0
my_DataADC2__1_L1:

	LDMIA R1!, {R4-R7}		//  4 слова = 8 полуслов = 2 отсчета
    AND R10, R4, R8			// подготовили младшее полуслово
    ADD R9, R9, R10			// суммировали младшее полуслово
	LSR R4, R4, #16			// переместили в R4 старшее полуслово на место младшего OUT2
	AND R10, R5, R8			// подготовили младшее полуслово
	ADD R9, R9, R10			// суммировали младшее полуслово
	LSR R5, R5, #16			// переместили в R5 старшее полуслово на место младшего OUT1
	AND R10, R6, R8			// подготовили младшее полуслово
	ADD R9, R9, R10			// суммировали младшее полуслово
	LSR R6, R6, #16			// переместили в R6 старшее полуслово на место младшего OUT2
	AND R10, R7, R8			// подготовили младшее полуслово
	ADD R9, R9, R10			// суммировали младшее полуслово
	LSR R7, R7, #16			// переместили в R7 старшее полуслово на место младшего OUT1
...
//	разбор буфера DMA1_2 завершен
    ...
    LDR R1, =sum_OUT_0R
    LDR R4, [R1]			// загрузили прежнюю сумму sum_OUT_0R
    ADD R4, R4, R9
    STR R4, [R1]			// сохранили обновленную сумму sum_OUT_0R
//	===========================
    LDMIA  SP!, {R0 - R10}
    BX  LR
.endfunc
...
my_F1:
	...
	//LDR R0, =my_DMA2_Data_F0	// адрес начала входного буфера ТАК БЫЛО
	LDR R0, =my_Data_F0			// адрес ячейки с адресом начала входного буфера
	LDR R0, [R0]				// адрес начала входного буфера ТАК СТАЛО
	ADD R0, R0, #52				// 52 = 26*2
	ADD R1, R0, #60				// 60 = 30*2
    //LDR R2, =my_DMA2_Data_F1	// адрес начала массива для результата ТАК БЫЛО
    LDR R2, =my_Data_F1	// адрес ячейки с адресом начала массива для результата
    LDR R2, [R2]				// адрес начала массива для результата ТАК СТАЛО
...
my_F2:
	...
	MOV LR, #25					// 150 = 6*25
	//
	//LDR R0, =my_DMA2_Data_F2	//	адрес начала массива для результата ТАК БЫЛО
	LDR R0, =my_Data_F2			//
	LDR R0, [R0]				// ТАК СТАЛО
	//LDR R1, =my_DMA2_Data_F0	//	адрес начала (120+56)*3*(32 бит)    ТАК БЫЛО
	LDR R1, =my_Data_F0			//
	LDR R1, [R1]				// ТАК СТАЛО
	//
...
my_F1F2:
	...
	//LDR R1, =my_DMA2_Data_F1	// адрес начала (120)*(16 бит)	 ТАК БЫЛО
	LDR R1, =my_Data_F1			//
	LDR R1, [R1]				// ТАК СТАЛО
...
my_F1F2_L1:
	...
	SUB  R2, R0, R2
	ADD  R10, R10, R2		// сумматор №3 ОШИБКА (отличие на 1)
	...
	SUB  R2, R0, R2
	ADD  R12, R12, R2		// сумматор №5 ОШИБКА (отличие на 2)
	//
	//
	//
	LDMIA  SP!, {R0}
	...
	BNE my_F1F2_L1
	// =========================
	//LDR R2, =my_F1F2_rez	//	адрес сумматора разностей №1 (далее до №5)	ТАК БЫЛО
...
my_F2F1:
	...
	// =========================
	MOV R0, #10
	//LDR R1,  =my_DMA2_Data_F2	//	адрес начала (120+30)*(16 бит)					ТАК БЫЛО
	LDR R1,  =my_Data_F2		//
	LDR R1, [R1]				// 													ТАК СТАЛО
...
my_F2F1_L1:
	...
	BNE my_F2F1_L1		// 2
	// =========================
	//LDR R2, =my_F2F1_rez	//	адрес сумматора разностей №1 (далее до №5)	ТАК БЫЛО
...
my_F1_F2:
	...
	//LDR R0, =my_DMA2_Data_F1_F2	//	адрес начала массива для результата		ТАК БЫЛО
