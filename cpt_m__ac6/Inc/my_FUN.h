...
void my_tst_1(void);
void my_F0_S(void);
void my_DataADC__0_(void);	// пересылка из буфера pDataDMA в буфер pDataADC
void my_DataADC1__1_();		// разбор данных ADC1
void my_DataADC2__1_();		// разбор данных ADC2
void my_DataADC3__1_();		// разбор данных ADC3
...
void my_DataADC2_3(void);	// разбор буфера на 4 части
...
void my_F1F2(void);			// синхронный детектор частоты F2 (12 кГц) после режектирования F1 (10 кГц), вычисление 1-й производной СПЕКТРА ПРОПУСКАН� Я (дискриминатор ДЛ� НЫ ВОЛНЫ)
void my_F1F2_P(void);		// находим суммы для частоты F2 (12 кГц) для вычисления производной дискриминатора ДЛ� НЫ ВОЛНЫ (2-й производной СПЕКТРА ПРОПУСКАН� Я)
void my_F2(void);			// режекция частоты F2 (период 25 = 5*5, 5 полупериодов по 5 отсчетов, период 10, частота 12 кГц - модуляция ДЛ� НЫ ВОЛНЫ)
